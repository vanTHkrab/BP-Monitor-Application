...
Canonical agent-facing file for `server/app/api-gateway/`. `CLAUDE.md` is a
`@AGENTS.md` pointer. Supplements the root [AGENTS.md](../../../AGENTS.md).

## What this service is

NestJS 11 + Fastify + Mercurius (GraphQL) gateway, Prisma 7 → PostgreSQL.
The single entry point for the mobile client, and **the owner of all durable
shared state in the project**. Handles auth, persistence, S3 upload
presigning, and bridging to the FastAPI AI service over Redis.
...
> **Note:** the web dashboard is *not* meaningfully a client of this service.
> It calls one unauthenticated `hello` query as a liveness probe and reaches
> Postgres, Redis, and S3 directly. A schema change ships to one real client
> (`client/`), but a **datastore** change ships to two consumers. See the root
> [AGENTS.md](../../../AGENTS.md) topology diagram.

## Skills to load before working here

Vendored under `.agents/skills/`, versioned against what this service runs:

| Working on | Load |
| --- | --- |
| Prisma schema or migrations | `prisma-cli`, `prisma-database-setup` |
| Prisma queries | `prisma-client-api` |
| Prisma 7 upgrade fallout | `prisma-upgrade-v7` |
| Redis keys, TTLs, Lua | `redis-core`, `redis-connections` |
| Redis in production | `redis-security`, `redis-observability` |

Scaffold new modules with the `nest` CLI (`nest g module|resolver|service`) —
it wires the DI graph and generates spec files. It does not *move* existing
files; reorganising inside a module is a manual refactor.
...
pnpm exec jest --watchman=false   # unit: 18 suites / 181 tests. NOT `pnpm test`
...
- **Add a service-level unit test with new behavior.** The suite is 18 files
  / 181 tests and `auth/` is covered (`auth.service.spec.ts`,
...
- **The web dashboard is not a second GraphQL client.** It calls only the
  public `hello` query, as a liveness probe, with no token
  ([web/src/lib/gateway.ts](../../../web/src/lib/gateway.ts)). A breaking
  schema change therefore ships to **one** client — the Expo app. What the
  dashboard *does* depend on is the raw shape of Postgres, Redis, and S3,
  which it reads directly. So a Prisma migration can break the dashboard
  without touching a single resolver, and the gateway's test suite will not
  catch it.
...
- [ARCHITECTURE.md](./ARCHITECTURE.md) — request lifecycle, auth state machine, module dependency graph (Thai)
- [README.md](./README.md) — onboarding & ops
- [MEMORY.md](./MEMORY.md) — durable facts worth remembering across sessions
- [docs/reference/API.md](../../../docs/reference/API.md) — GraphQL contract for client developers (auth, error codes, operation catalogue, image-upload flow)
- [docs/project/api-gateway-plan.md](../../../docs/project/api-gateway-plan.md) — roadmap and known gaps
- [docs/architecture/AUTH-better-auth-identity.md](../../../docs/architecture/AUTH-better-auth-identity.md) — the identity model and why Better Auth
- [docs/guides/troubleshooting.md](../../../docs/guides/troubleshooting.md) — validation, GraphiQL, and passkey failures
