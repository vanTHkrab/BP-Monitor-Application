...
| 27 | Wire gateway side for ai-service real OCR | ☐ | When [ai-service/PLAN.md](../ai-service/PLAN.md) lands the real YOLO + ssocr pipeline, 5 files in `src/ai/` need synced changes: `types/ai.types.ts` adds `imageUrl` to job payload; `ai.service.ts` calls `S3StorageClient.presignGet` and includes URL; `ai.module.ts` re-imports `StorageModule`; `ai.process.ts` accepts `model_version` + `roi_image_bytes` (base64), uploads ROI image to S3 from this worker (gateway owns S3 creds); `dto/analysis-job.object.ts` adds `modelVersion`. See ai-service PLAN's "Cross-cutting changes on the gateway" section |
