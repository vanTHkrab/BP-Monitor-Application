...
| `models/yolo26n-*.onnx`, `models/yolo11n-adam-color.onnx` | The detector comparison set, same 5 classes and 512 input as `yolo11n.onnx`. The `yolo26n-*` three are **end-to-end exports** — output is `[1, 300, 6]` rows of `(x1, y1, x2, y2, conf, cls)`, already suppressed, so `iou_threshold` is inert for them; `yolo11n-adam-color.onnx` is an anchors export like `yolo11n.onnx`. ~10 MB each, fetched from R2 like the rest. A `gray` marker in the filename means the model was trained on grayscale renders and still takes **3-channel** input, so the caller replicates gray across the channels — `infer_grayscale_input` reads that marker, and feeding a `-gray` model colour is an accuracy loss with no symptom. **None is bundled in the mobile app** or listed in `verify-models.mjs`; the phone still loads `yolo11n.onnx`, and both sides dispatch on the graph shape precisely so the two can differ during the transition. Switching `AI_DETECTOR_PATH` to any of these without shipping the matching mobile copy breaks ADR-002's premise: the phone would gate framing with a detector that labels the fields by position while the server uses one that labels by content. |
