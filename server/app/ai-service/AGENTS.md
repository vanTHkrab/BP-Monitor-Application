...
| `models/yolo26n-adamw-color.onnx` | **Default `AI_DETECTOR_PATH`.** YOLO26n detector, 5 BP-specific classes, end-to-end export (NMS embedded in the graph — `iou_threshold` is inert for it), 512 input, 10,068,777 bytes (~9.6 MB). **Fetched from R2 at startup**, not tracked in git. **Also bundled verbatim in the mobile app** at `client/assets/models/yolo26n-adamw-color.onnx` for on-device pre-flight (see [client/AGENTS.md](../../../client/AGENTS.md)). The canonical sha256 lives in `EXPECTED_HASHES.json`; when you retrain, regenerate the manifest, upload the new bytes to R2, and refresh the mobile copy in the same change — `client/scripts/verify-models.mjs` runs on every `pnpm start` and fails the dev build on SHA256 drift. |
| `models/yolo11n.onnx`, `models/yolo26n-adamw-gray.onnx`, `models/yolo26n-spg-gray.onnx`, `models/yolo11n-adam-color.onnx` | The detector comparison set — no longer the live default, same 5 classes and 512 input as `yolo26n-adamw-color.onnx`. `yolo11n.onnx` and `yolo11n-adam-color.onnx` are **anchors exports** (`nms=False`, NMS owed to the Python decoder) like the old default; the two `yolo26n-*-gray` files are **end-to-end exports** like the current default, so `iou_threshold` is inert for them too. A `gray` marker in the filename means the model was trained on grayscale renders and still takes **3-channel** input, so the caller replicates gray across the channels — `infer_grayscale_input` reads that marker, and feeding a `-gray` model colour is an accuracy loss with no symptom. ~10 MB each, fetched from R2 like the rest. **None of these four is bundled in the mobile app** or listed in `verify-models.mjs`. Switching `AI_DETECTOR_PATH` to any of these away from the shipped default without shipping the matching mobile copy reopens ADR-002's premise gap: the phone would gate framing with a detector that labels the fields by position while the server uses one that labels by content. |
...
The mobile app verifies its bundled copies of both `yolo26n-adamw-color.onnx`
and `crnn.onnx` directly against the hashes in this manifest
