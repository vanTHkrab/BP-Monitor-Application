...
export const initLocalDb = async (): Promise<void> => {
  ...
  for (const row of missingReadings ?? []) {
    ...
    await db.runAsync(`UPDATE pending_readings SET clientId = ? WHERE id = ?;`, [clientId, row.id]);
  }
  ...
  for (const row of missingPosts ?? []) {
    ...
    await db.runAsync(`UPDATE local_posts SET clientId = ? WHERE id = ?;`, [clientId, row.id]);
  }
};

export const insertPendingReading = async (row: Omit<PendingReadingRow, 'id'>): Promise<number | null> => {
  const db = await getDb();
  if (!db) return null;
  ...
  return typeof result.lastInsertRowId === 'number' ? result.lastInsertRowId : null;
};

export const listPendingReadings = async (userId: string): Promise<PendingReadingRow[]> => {
  const db = await getDb();
  if (!db) return [];
...
export const insertLocalPost = async (row: Omit<LocalPostRow, 'id'>): Promise<number | null> => {
  const db = await getDb();
  if (!db) return null;
  const result = await db.runAsync(
    `INSERT INTO local_posts (userId, clientId, userName, userAvatar, content, category, createdAt)
     VALUES (?, ?, ?, ?, ?, ?, ?);`,
    [row.userId, row.clientId, row.userName, row.userAvatar, row.content, row.category, row.createdAt]
  );
  return typeof result.lastInsertRowId === 'number' ? result.lastInsertRowId : null;
};

export const listLocalPosts = async (userId: string): Promise<LocalPostRow[]> => {
  const db = await getDb();
  if (!db) return [];
...
export const updateLocalPost = async (id: number, input: { content?: string; category?: string }): Promise<void> => {
  const db = await getDb();
  if (!db) return;
  ...
  await db.runAsync(`UPDATE local_posts SET ${updates.join(', ')} WHERE id = ?;`, [...values, id]);
};
...
export const queuePendingPostAction = async (row: Omit<PendingPostActionRow, 'id'>): Promise<void> => {
  const db = await getDb();
  if (!db) return;
  await db.runAsync(`DELETE FROM pending_post_actions WHERE userId = ? AND postId = ?;`, [row.userId, row.postId]);
  await db.runAsync(
    `INSERT INTO pending_post_actions (userId, postId, action, content, category, updatedAt)
     VALUES (?, ?, ?, ?, ?, ?);`,
    [row.userId, row.postId, row.action, row.content, row.category, row.updatedAt]
  );
};

export const listPendingPostActions = async (userId: string): Promise<PendingPostActionRow[]> => {
  const db = await getDb();
  if (!db) return [];
