...
export const loadReminderSettings = async (userId?: string) => {
  const parsed = await kvStorage.getJSON<Partial<ReminderSettings>>(
    userKey.reminderSettings(userId),
  );
...
export const saveReminderSettings = async (
  ...
  await kvStorage.setJSON(userKey.reminderSettings(userId), settings);
};
