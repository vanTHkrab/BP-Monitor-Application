import { IMAGE_CACHE_TTL_MS } from "@/src/core/config/constants";
import {
    deleteCachedImage,
    getCachedImage,
...
export const resolveImageUri = async (
  ...
  if (existing) {
    ...
    if (file.exists && ageMs < IMAGE_CACHE_TTL_MS) {
      return file.uri;
    }
...
// older than IMAGE_CACHE_TTL_MS plus the file it points at. Safe to call
// from anywhere — never throws.
export const cleanupExpiredImages = async (): Promise<void> => {
  if (Platform.OS === "web") return;
  try {
    const cutoff = new Date(Date.now() - IMAGE_CACHE_TTL_MS).toISOString();
