import { GQL_ERROR_CODES } from "@/src/core/config/constants";
import {
    errorCode,
    errorHttpStatus,
    errorRetryAfterSec,
...
export const formatAuthError = (
  ...
  if (code === GQL_ERROR_CODES.NETWORK_TIMEOUT) {
    return {
      message:
  ...
  if (code === GQL_ERROR_CODES.NETWORK_FAILED) {
    return {
      message:
  ...
  if (code === GQL_ERROR_CODES.UNAUTHENTICATED) {
    return {
  ...
  if (code === GQL_ERROR_CODES.FORBIDDEN) {
    return {
  ...
  if (code === GQL_ERROR_CODES.CONFLICT) {
    return {
      message:
  ...
  if (code === GQL_ERROR_CODES.BAD_USER_INPUT) {
