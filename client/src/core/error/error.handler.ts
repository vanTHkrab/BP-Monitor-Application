...
//
// `formatAuthError` (in `store/shared/error-format`) is the auth-flow
// counterpart — it knows about login throttle countdowns and field-level
// hints. Use this `formatError` for everything else (banners, toasts,
// inline messages outside auth screens).
...
const toUserMessage = (raw: string, fallback: string): string => {
  ...
  // core/graphql/client.ts surfaces as `[CODE] message`. Match those alongside
  // the REST-style status patterns.
