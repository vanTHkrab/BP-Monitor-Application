...
| `hooks/use-camera-analysis.ts` | State machine for BP image capture → AI analysis → save. `runPreflight()` still exists and can run the on-device YOLO check via `state.preflight`, but `app/(tabs)/camera.tsx` no longer calls it (see `services/preflight-detection.service.ts` above). `analyze()` is unchanged — the camera screen calls it directly after capture. `save()` delegates to `readings.slice.createReading` so the camera flow inherits the offline queue and optimistic UI used by manual entry. |
...
- **On-device pre-flight (currently bypassed in the UI)**: BP image
  ...
  `app/(tabs)/camera.tsx` calls `analyze()` directly after capture and
  does not call `preflightCheckImage` / `runPreflight` — there is no
