...
| `modules/bp-vision/BPVisionCameraView.tsx` | Thin RN wrapper around the native CameraX preview view (Android-only) via `requireNativeView`, backed by `android/.../BPVisionCameraView.kt` + `CameraController.kt`. Exposes the same `capture()` ref contract as `expo-camera`'s `takePictureAsync` (upright JPEG `{ uri, width, height }`) so `utils/crop-to-viewport.ts` needs no changes, plus the `liveDetection` prop / `onDetections` event that drive the live framing gate (see below). Consumed only through `components/bp-camera-view.tsx` — don't render it directly, it isn't registered on iOS/web. |
| `utils/framing-state.ts` | Pure logic for the live framing gate: `evaluateFraming` classifies one detector frame into `searching` / `too-far` / `too-close` / `off-center` / `ready`, and `advanceHysteresis` smooths that over a **time-based** dwell (`FRAMING_DWELL_MS`, not a frame count — measured analysis frame rate drifts with scene and thermals, so a frame count would mean a different wall-clock delay per device). Thresholds live in `DEFAULT_FRAMING_THRESHOLDS`; they are provisional and are the intended tuning surface. Unit-tested in `__tests__/framing-state.test.ts` — this is the rule that decides when the shutter may fire automatically, so it is deliberately assertable rather than only observable on a device. |
| `hooks/use-live-framing.ts` | React side of the framing gate: feeds frames through `utils/framing-state`, then arms / counts down / fires auto-capture and cancels it. `AUTO_COUNTDOWN_MS` is 1.5 s (2.5 s when a screen reader is on, since the ring is a purely visual cue). Cancelling suppresses re-arming until the framing leaves `ready` at least once, so a cancel isn't immediately undone by a phone still pointed at the monitor. |
| `utils/detect-benchmark.ts` | `__DEV__`-only latency benchmark for the on-device detector, surfaced by `DetectBenchmarkChip` in `components/dev-ocr-controls.tsx` (gated on `__DEV__`, **not** `devMode` — it is a build-time instrument, not a demo toggle). Reports median / p90 / fps **and the detected class names**, because a smaller input size that raises frame rate while quietly losing the `sys`/`dia`/`pulse` boxes is a regression, not a win. |
| `lib/yolo/` | **Mostly dead JS-side YOLO scaffolding, kept as inert reference code, not a working pre-flight path.** `types.ts` is the one live file — it mirrors the backend class layout (`0 BP_Monitor` / `1 BP_Screen_Monitor` / `2 dia` / `3 pulse` / `4 sys`) and is imported by the native module's TS wrapper (`modules/bp-vision/index.ts`); keep it in sync with `server/app/ai-service/src/ai_service/analyzer/yolo.py::CLASS_NAMES`. `postprocess.ts` (Ultralytics-style `[1, 4+C, anchors]` decode + per-class NMS) is orphaned — imported by nothing. `preprocess.ts` (letterbox + JPEG-decode → `[1,3,512,512]` float32 RGB) has been **deleted** — it was the only consumer of `jpeg-js`, so keeping it as reference code meant carrying `@types/jpeg-js` + `jpeg-js` in the manifest for dead code. `detect.ts`, the orchestrator that once tied preprocess/session/postprocess together, is entirely commented out. `session.ts` (the `onnxruntime-react-native` `InferenceSession` loader) was deleted outright, not just unwired — that package was dropped as a dependency in an earlier commit on this line of work, so the file's import was permanently broken, not a working revert target. On-device YOLO inference now runs exclusively through the native Kotlin `bp-vision` module. |
...
| `store/slices/` | Domain slices: `auth` (+ sessions), `profile` (me + avatar queue), `readings` (+ alerts), `community` (posts + comments), `caregivers`, `preferences` (theme + font + security + `autoCaptureEnabled`), `network`. |
...
- **JS-side on-device pre-flight has been removed (not just bypassed)**: BP
  ...
  revert." `lib/yolo/preprocess.ts` was deleted for the same reason —
  it was the only consumer of `jpeg-js`, so keeping dead code meant
  keeping a dependency for it. `lib/yolo/detect.ts` (now fully commented
  out), `postprocess.ts`, and
  ...
  shape. The backend YOLO pass still does its own ROI detection
  ...
  section in the root `CLAUDE.md`). The live framing gate described below
  is the replacement, built against that native module — do not revive
  this JS path.
- **Live framing gate + auto-capture (Android, real builds only)**: the
  camera screen runs YOLO on the CameraX preview stream and coaches the
  user toward a usable shot. `CameraController.kt` binds an
  `ImageAnalysis` use case (`KEEP_ONLY_LATEST`, `RGBA_8888`, its own
  executor so inference can never delay a capture); `BPVisionCameraView.kt`
  runs the detector and emits throttled `onDetections`; `bp-camera-view.tsx`
  forwards it, and `hooks/use-live-framing.ts` turns the stream into a
  smoothed state that colours the guide frame and drives one line of Thai
  coaching copy. Measured **~4.0 fps end-to-end** on a mid-range physical
  device (≈250 ms/frame: ~183 ms inference + conversion); the ImageAnalysis
  frame is cropped, rotated, and downscaled to the detector's input in a
  **single** `Matrix` pass, because doing those separately allocated two
  extra full-resolution bitmaps per frame. Analysis and preview are both
  pinned to **16:9** so they share a field of view — detections are drawn
  over the preview and a mismatched FOV offsets every box.
  **Three rules this design is built on, don't quietly break them:**
  (1) the gate is a *soft nudge* — the shutter is never blocked by it, so a
  detector false negative can't stop someone recording a reading;
  (2) "monitor present" accepts `BP_Monitor` **or** `BP_Screen_Monitor` —
  the outer box is the first to drop out at harder framings, so requiring
  class 0 would report "nothing here" over a plainly visible display;
  (3) auto-capture calls the same `takePicture()` as the button, so the
  automatic path can't drift from the manual fallback. iOS / web / Expo Go
  have no detector: `onDetections` never fires, the state stays
  `searching`, and the screen behaves exactly as it did before this
  existed — that degraded mode is supported, not an error case.
  `autoCaptureEnabled` (preferences slice, **default on**, toggled from the
  camera screen itself) gates only the countdown, never the coaching.
