...
- [ ] **A-008** `low` Decide whether the shared rate limiter should slide instead of jump — `redis/rate-limit.service.ts` is a fixed window, so any budget built on it (Better Auth's credential endpoints, A-007's invite throttle) allows a boundary burst: the full budget just before the window rolls plus the full budget just after. A sliding window (sorted-set of timestamps, or two-bucket weighted) closes it at the cost of more Redis work per call. Deliberately not done inside A-007 — the primitive is shared, so changing it changes login-throttle behaviour too, and that deserves its own review
