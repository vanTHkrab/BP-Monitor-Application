...
description: >-
    ...
    Postgres via Prisma and bridges to the FastAPI AI service over a Redis
    pub/sub contract. Media bytes flow client → S3 directly via presigned PUT.
status: current
updated: 2026-08-06
owner: cross
---
...
Edges are direct calls or pub/sub hops. Postgres is the only durable store;
Redis is a transport, not a database; SQLite on mobile is a queue + mirror, not
authority.
...
- **One gateway, one client** — The mobile app hits the GraphQL endpoint with a
  token auth scheme. Any breaking schema change ships straight to the
  patient-facing client.
- **Redis is the only AI contract** — Channels analyze_bp_image and
  analyze_bp_image.reply, with payload shapes owned by
  api-gateway/src/ai/ai.process.ts and mirrored by
...
- **Redis is optional at boot** — The gateway lazy-connects and degrades to an
  in-memory throttle and a disabled AI path if Redis is down. AI features fail
  soft; auth keeps working.
...
- **Postgres as the only source of truth** — SQLite (mobile) and Redis
  (transport) are caches/queues, not authority. The mobile store writes
  optimistically, queues to SQLite (pending_readings) on failure, and
  reconciles on the next fetchX call; we accept some staleness for offline
  resilience.
